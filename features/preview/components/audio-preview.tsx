...
export function AudioPreviewContent({
  instrument,
  octaveRange,
}: AudioPreviewContentProps): React.JSX.Element {
  const { playbackChip, setPlaybackChip } = useSynthSettings();
  const displayedWhiteKeyCount = 7 * octaveRange;
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            onClick={() =>
              setOctaveOffset(
                (prev) => clampOctave(prev + octaveRange) - octaveRange + 1
              )
            }
            disabled={!withinOctave(octaveOffset)}
          >
